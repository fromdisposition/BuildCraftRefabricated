...
A robot's job comes from its <bold>board</bold>, written at the <link inline="buildcraft:block/programming_table"/>: Picker, Carrier, Farmer, Lumberjack, Miner, Butcher, Builder and more. Fuse that programmed board into a blank robot at an <link inline="buildcraft:block/integration_table"/> to assemble a loaded robot, then right-click it onto a docking station clipped to a pipe. Give that station power (MJ, or RF) and the robot runs its task, then returns to recharge. A robot hands its cargo to the pipe as if it arrived from the face opposite the station, so that face must be connected and the chest or pipe that should receive the cargo goes on a third face; the chest it takes tools and supplies from sits directly behind the station. See <link to="buildcraft:concept/silicon_and_chipsets"/> for programming the boards.
