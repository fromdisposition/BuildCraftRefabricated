...
The large slot on the <bold>left</bold> is the target item — the gate, chipset, blank robot, or other base you want to upgrade.
...
This is also where robots are built: a blank robot in the target slot plus a programmed board becomes a loaded robot ready to deploy — see <link to="buildcraft:concept/robots_and_zones"/>.
