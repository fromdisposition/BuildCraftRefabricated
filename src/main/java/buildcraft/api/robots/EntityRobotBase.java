...
public abstract class EntityRobotBase extends LivingEntity {
   ...
   /**
    * Leash radius (blocks) from the robot's home station for searches with no explicit zone. Measured from the
    * station, not the robot's current position, so it is a true leash the robot cannot drift past. A Zone Planner
    * work area overrides it, see {@link #ZONE_SEARCH_RANGE}.
    */
   public static final float DEFAULT_SEARCH_RANGE = 32.0F;
   /**
    * Coarse query bound (blocks) used only when a Zone Planner work area is set; the real clamp is {@code zone
    * contains()}, but lookups still need a finite box, so a large drawn zone isn't clipped to {@link #DEFAULT_SEARCH_RANGE}.
    */
...
/*super(EntityType.PIG, par1World);
   ...
   /**
    * Profile of the player who deployed this robot, or {@code null} for legacy robots saved before owner
    * tracking existed. Threaded into world-modifying actions so land-claim protection can authorize the robot
    * as it would the owning player; a {@code null} owner falls back to {@code BuildCraftAPI.fakePlayerProvider},
    * which fails closed inside any claim.
    */
   @Nullable
   ...
   /** Centre of the robot's home station, the leash origin for searches; falls back to the robot's own position if unlinked (e.g. mid-reload). */
