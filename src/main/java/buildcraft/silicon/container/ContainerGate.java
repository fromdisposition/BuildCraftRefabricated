...
public class ContainerGate extends BcMenu {
   ...
   public ContainerGate(int containerId, Inventory playerInv, PluggableGate pluggable) {
      ...
      if (pluggable == null) {
         ...
         // This side can fail to resolve the gate while the other side (which opened the menu) has one; the slot
         // list must still match the server's or the client menu ends up slot-less and disconnects.
