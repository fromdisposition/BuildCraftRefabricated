...
public record BcHeatWhiteSpriteSource(Identifier source) implements SpriteSource {
   public static final Identifier ID = Identifier.fromNamespaceAndPath("buildcraftenergy", "heat_white");
   public static final MapCodec<BcHeatWhiteSpriteSource> MAP_CODEC = RecordCodecBuilder.mapCodec(
      i -> i.group(Identifier.CODEC.fieldOf("source").forGetter(BcHeatWhiteSpriteSource::source)).apply(i, BcHeatWhiteSpriteSource::new)
   );
   ...
   public void run(ResourceManager rm, Output output) {
      ...
      if (sourceRes.isEmpty()) {
         ...
         Identifier outputId = this.source.withSuffix("_white");
         LazyLoadedImage baseImage = new LazyLoadedImage(sourceTexId, sourceRes.get(), 1);
         output.add(outputId, new BcHeatWhiteSpriteSource.Loader(baseImage, sourceRes.get(), outputId));
      }
   }
   ...
   public static SpriteContents createWithSourceMetadata(Identifier outputId, NativeImage image, Resource sourceResource) {
      Optional<AnimationMetadataSection> animationInfo = Optional.empty();
      Optional<TextureMetadataSection> textureInfo = Optional.empty();
      List<WithValue<?>> additionalMetadata = List.of();
      ...
      FrameSize frameSize = animationInfo.<FrameSize>map(animation -> animation.calculateFrameSize(image.getWidth(), image.getHeight()))
   ...
   private record Loader(LazyLoadedImage base, Resource sourceResource, Identifier outputId) implements DiscardableLoader {
      public @Nullable SpriteContents get(SpriteResourceLoader loader) {
         ...
         try {
            ...
            for (int y = 0; y < totalH; y++) {
               for (int x = 0; x < frameW; x++) {
                  ...
                  if (a == 0) {
                     ...
                     out.setPixel(x, y, a << 24 | 16777215);
                  }
               }
            }
            ...
            BcHeatWhiteSpriteSource.LOGGER.error("BcHeatWhiteSpriteSource: failed to generate {}", this.outputId, e);
            return null;
         } finally {
            if (out != null) {
               out.close();
