...
public abstract class ContainerElectricEngine<T extends TileElectricEngineBase> extends BcMenu {
   ...
   protected ContainerElectricEngine(MenuType<?> type, int containerId, Inventory playerInv, final T engine, ToLongFunction<T> power, int upgradeSlotX) {
      super(type, containerId, playerInv.player);
      this.engine = engine;
      ...
      for (int slot = 0; slot < 4; slot++) {
         this.addSlot(new SlotBase(upgrades, slot, upgradeSlotX + 18 * slot, 44) {
            @Override
