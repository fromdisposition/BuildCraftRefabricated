...
public class OilGenerator {
    ...
    public static boolean canGenerateOilIn(ServerLevel level) {
        ...
        if (level.dimension() == Level.NETHER && BCEnergyConfig.enableNetherOilGeneration.get()) {
            return true;
        }
    ...
    public static boolean wouldGenerateOilForOriginChunkInOilBiome(ServerLevel level, int chunkX, int chunkZ) {
        ...
        if (!isChunkLoaded(level, chunkX, chunkZ)) return false;
    ...
    public static boolean isOilDesignBiomeAt(Level level, int chunkX, int chunkZ) {
        if (level instanceof ServerLevel server && !isChunkLoaded(server, chunkX, chunkZ)) {
            return false;
        }
    ...
    private static boolean isChunkLoaded(ServerLevel level, int chunkX, int chunkZ) {
        BlockPos center = new BlockPos(chunkX * 16 + 8, level.getMinY(), chunkZ * 16 + 8);
        return level.hasChunkAt(center);
    }
    ...
    private static List<OilGenStructure> getNetherStructures(
            Level level, int cx, int cz, boolean log, Random rand, int x, int z) {
        double globalMul = BCEnergyConfig.oilWellGenerationRate.get()
                * BCEnergyConfig.netherOilGenRateMultiplier.get();

        final GenType type;
        if (rand.nextDouble() <= BCEnergyConfig.largeOilGenProb.get() * globalMul) {
            type = GenType.LARGE;
        } else if (rand.nextDouble() <= BCEnergyConfig.mediumOilGenProb.get() * globalMul) {
            type = GenType.MEDIUM;
        } else {
            if (DEBUG_OILGEN_ALL & log) {
                BCLog.logger.info("[energy.oilgen] Not generating nether oil in chunk " + cx + ", " + cz
                        + " because none of the random numbers were above the thresholds.");
            }
            return ImmutableList.of();
        }
        ...
        if (DEBUG_OILGEN_BASIC & log) {
            BCLog.logger.info("[energy.oilgen] Generating nether oil well (" + type.name().toLowerCase(Locale.ROOT)
                    + ") in chunk " + cx + ", " + cz + " at " + x + ", " + z);
        }
        ...
        if (BCEnergyConfig.enableOilSpouts.get()) {
            ...
            int height = maxHeight == minHeight
                    ? maxHeight
                    : minHeight + rand.nextInt(Math.max(1, maxHeight - minHeight));
