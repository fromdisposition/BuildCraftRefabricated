...
public enum CoreTriggerProvider implements ITriggerProvider {
    ...
    public void addExternalTriggers(Collection<ITriggerExternal> res, @Nonnull Direction side, BlockEntity tile) {
        ...
        if (!blockInventoryTriggers && tile != null && tile.getLevel() != null) {
            ResourceHandler<ItemResource> itemHandler = buildcraft.lib.attachments.AttachmentQueries.getBlock(
                    tile.getLevel(), Attachments.Item.BLOCK, tile.getBlockPos(), side.getOpposite());
        ...
        if (!blockFluidHandlerTriggers && tile != null && tile.getLevel() != null) {
            ResourceHandler<FluidResource> fluidHandler = buildcraft.lib.attachments.AttachmentQueries.getBlock(
                    tile.getLevel(), Attachments.Fluid.BLOCK, tile.getBlockPos(), side.getOpposite());
