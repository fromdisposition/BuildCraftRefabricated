...
public enum PipeRegistry implements IPipeRegistry {
    ...
    public IItemPipe createItemForPipe(PipeDefinition definition) {
        ...
        ItemPipeHolder item = BCRegistries.registerItem(
                id.getNamespace(),
                "pipe_" + id.getPath(),
                props -> new ItemPipeHolder(BCTransportBlocks.PIPE_HOLDER.get(), definition, props));
    ...
    public IItemPipe createUnnamedItemForPipe(PipeDefinition definition, Consumer<Item> postCreate) {
        ItemPipeHolder item = new ItemPipeHolder(
    ...
    private static Identifier parseIdentifier(String identifier) {
        ...
        if (colon >= 0) {
            return Identifier.fromNamespaceAndPath(identifier.substring(0, colon), identifier.substring(colon + 1));
        }
        return Identifier.fromNamespaceAndPath(BCTransport.MODID, identifier);
    }
