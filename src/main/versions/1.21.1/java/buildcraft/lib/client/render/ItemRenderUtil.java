...
public class ItemRenderUtil {
   ...
   public static void beginItemBatch(PoseStack poseStack, MultiBufferSource bufferSource, int light) {
   ...
   public static void renderItemStack(double x, double y, double z, ItemStack stack, int stackCount, int lightc, Direction dir, VertexConsumer bb) {
      if (!stack.isEmpty() && currentPoseStack != null && currentBufferSource != null) {
         if (dir == null) {
            dir = Direction.EAST;
         }
         ...
         for (int i = 0; i < itemModelCount; i++) {
            ...
            Minecraft.getInstance().getItemRenderer().renderStatic(
               stack, ItemDisplayContext.FIXED, lightc, OverlayTexture.NO_OVERLAY, currentPoseStack, currentBufferSource, Minecraft.getInstance().level, 0
            );
