...
public enum PlugBakerFacade implements IPluggableStaticBaker<KeyPlugFacade> {
   ...
   private static List<BakedQuad> getQuadsFromModel(BakedModel model, BlockState state, Direction side) {
   ...
   private List<MutableQuad> getTransformedQuads(BlockState state, BakedModel model, Direction side, Vec3 pos0, Vec3 pos1, Vec3 pos2, Vec3 pos3) {
      return getQuadsFromModel(model, state, side)
         .stream()
         .map(
            quad -> {
   ...
   private void addRotatedQuads(
      List<MutableQuad> quads, BlockState state, BakedModel model, Direction side, int rotation, Vec3 pos0, Vec3 pos1, Vec3 pos2, Vec3 pos3
   ) {
      quads.addAll(
